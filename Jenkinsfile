pipeline {
    ...
    parameters {
        ...
        string(name: 'SERVER_FQDN',
               defaultValue: 'ec2-3-84-200-167.compute-1.amazonaws.com',
    ...
    stages {
        ...
        stage('Base installation') {
            ...
            steps {
                ...
                sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'sudo wget -O /etc/nginx/sites-available/default https://raw.githubusercontent.com/J3GI/ci-cd-test/k8s/nginx.conf'"
        ...
        stage('Deploy with k8s on server') {
            steps {
                script {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'kubectl apply -f https://raw.githubusercontent.com/J3GI/ci-cd-test/k8s/rocketchat.yaml'"
                    sleep 90
